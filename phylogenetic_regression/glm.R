...
sink(file = "summary_glm.txt", append = TRUE)

for (colns in colnames(df_sub[29925:32547])) { 
  
  df_ind <- df_sub[c("pair_bond", colns)]
  df_ind["std_counts"] <- (df_ind[[colns]] - mean(df_ind[[colns]]))/sd(df_ind[[colns]])
  
  print(colns)
  print(summary(glm(pair_bond ~ std_counts, data = df_ind, family = 'binomial')))
